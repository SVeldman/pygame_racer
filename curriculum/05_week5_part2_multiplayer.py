"""Week 5, Part 2 - Multiplayer, built on top of Part 1's menu.

WHAT THIS FILE ADDS
--------------------
Built on top of Part 1's settings menu, this adds a second human player:
Player 1 (arrow keys) and Player 2 (W/A/S/D) each get their own stacked
band, and the menu now has a car-choice field for each of them. Both
players race the exact SAME race - the same AI racers, at the same
positions, crashing and recovering the same way no matter which band you're
watching them from - not two similar-looking but independent races.

If Part 1 made sense to you, this file has very few genuinely NEW ideas -
it's mostly "give the player dict a sibling, and run everything twice."
That's worth noticing: once `player` was a dict and `racers` was a list
built from menu settings (both already true in Part 1), adding a second
player is mostly copy-and-connect, not new invention. Good code design
pays off when you extend a feature, not just when you write it the first
time.

The one new wrinkle: `row_at`/`lane_to_x`/etc. already turn an absolute
track position into a spot on ONE player's band (that's what a single
player needed). Here we lean on that same math a second way: a single
shared racer still has just one absolute position, but we project it onto
each band separately, once per band per frame - so the exact same car ends
up drawn in the right (different) spot in both halves of the screen.

Run it with (from inside the `project` folder):
    pip install pgzero
    pgzrun 05_week5_part2_multiplayer.py
"""
